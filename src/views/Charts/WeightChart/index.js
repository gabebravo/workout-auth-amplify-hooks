...
const chartOptions = {
  ...
  subtitle: {
    text: 'Bench Press: max weight in pounds'
  },
  xAxis: {
    type: 'datetime',
...
const getMaxWeight = weightArr => weightArr
  .reduce( (acc, num) => {
...
const drawChart = ( exercises = []) => {
  if( exercises.length > 0 ) {
    const chartData = { 
      name: "max wieght",
      data: exercises.map( exercise => [ Date.UTC(...getDate(exercise.date)), getMaxWeight(exercise.weight)])
    }
